...
const STEP_STYLE = {
  PENDING: { bg: "bg-slate-100 text-slate-500", dot: "bg-slate-300", label: "Pending" },
  IN_PROGRESS: { bg: "bg-blue-100 text-blue-700", dot: "bg-blue-500 animate-pulse", label: "In Progress" },
  COMPLETED: { bg: "bg-emerald-100 text-emerald-700", dot: "bg-emerald-500", label: "Done" },
};

const PROD_STATUS_STYLE = {
  PLANNED: { bg: "bg-slate-100 text-slate-600", label: "Planned" },
...
function ScoreBar({ score }) {
  ...
  const labelColor = pct >= 80 ? "text-emerald-700 bg-emerald-100" : pct >= 40 ? "text-amber-700 bg-amber-100" : "text-red-700 bg-red-100";
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-sm font-bold text-slate-800 w-8 text-right">{pct}</span>
        <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${labelColor}`}>{label}</span>
      </div>
    </div>
  );
}
...
function ConfirmModal({ title, message, confirmLabel, confirmClass, onConfirm, onCancel, loading }) {
  return (
    <div className="fixed inset-0 z-[90] grid place-items-center bg-slate-900/40 px-4 animate-fade-in">
      <div className="w-full max-w-sm rounded-3xl bg-white p-7 shadow-2xl ring-1 ring-slate-900/10">
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-extrabold tracking-tight text-slate-900">Quality Control</h1>
            <p className="mt-1 text-sm text-slate-500">Inspect production runs, validate product quality, and manage certifications.</p>
          </div>
          <OrgSelector value={selectedOrgId} onChange={setSelectedOrgId} />
        </div>

        {error && <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700 font-medium">{error}</div>}

        {/* Summary KPIs */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: "Active Inspections", value: activeInspections.length, color: "text-blue-600", bg: "bg-blue-50" },
            { label: "Certified Products", value: certifiedCount, color: "text-emerald-600", bg: "bg-emerald-50" },
            { label: "Quality Issues", value: issuesCount, color: "text-red-600", bg: "bg-red-50" },
            { label: "Avg. AI Score", value: `${avgScore}`, color: "text-brand-600", bg: "bg-brand-50" },
          ].map((k) => (
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold transition-all ${
                activeTab === tab.key ? "bg-white text-brand-700 shadow-sm" : "text-slate-500 hover:text-slate-800"
              }`}
            >
              {tab.label}
              {tab.count !== null && (
              ...
              <div className="space-y-4">
                {activeInspections.length === 0 ? (
                  <div className="glass-card py-16 text-center">
                    ...
                    <p className="text-sm font-semibold text-slate-500">No active inspections.</p>
                    <p className="text-xs text-slate-400 mt-1">Start a production run from the Production page to begin inspection.</p>
                  </div>
                ) : (
                      ...
                      <article key={prod.id} className="glass-card p-6 space-y-5">
                        ...
                        <div className="flex items-start justify-between gap-4 flex-wrap">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap mb-1">
                              <span className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider bg-blue-100 text-blue-700">
                                <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />
                                In Progress
                              </span>
                              {score !== null && (
                                <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${score >= 80 ? "bg-emerald-100 text-emerald-700" : score >= 40 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}>
                                  AI Score: {score}
                                </span>
                              )}
                            </div>
                            <h3 className="text-base font-bold text-slate-900">{productName(prod.productId)}</h3>
                            <p className="text-xs text-slate-400 mt-0.5">{orgName(prod.organizationId)} · Qty {prod.quantity}</p>
                          </div>
                          <div className="flex gap-2 shrink-0">
                            <button
                              ...
                              className="rounded-full px-3 py-1.5 text-xs font-semibold bg-emerald-100 text-emerald-700 hover:bg-emerald-200 transition-colors"
                            >
                              Approve
                            </button>
                            <button
                              ...
                              className="rounded-full px-3 py-1.5 text-xs font-semibold bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                            >
                              Reject
                            </button>
                          </div>
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-1.5">
                            ...
                            <span className="text-xs font-bold text-slate-600">{completedSteps}/{totalSteps} steps · {pct}%</span>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                              {steps.map((step, idx) => {
                                ...
                                return (
                                  <div key={idx} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/50 px-4 py-3">
                                    <div className={`w-2 h-2 rounded-full shrink-0 ${ss.dot}`} />
                                    <div className="flex-1 min-w-0">
                                      <p className="text-sm font-semibold text-slate-800 truncate">{step.stepName}</p>
                                    ...
                                    <div className="flex gap-1.5 shrink-0">
                                      {step.status === "PENDING" && (
                                        <button
                                          ...
                                          className="rounded-lg px-2.5 py-1 text-[10px] font-bold bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors disabled:opacity-50"
                                        >
                                      ...
                                      {step.status === "IN_PROGRESS" && (
                                        <button
                                          ...
                                          className="rounded-lg px-2.5 py-1 text-[10px] font-bold bg-emerald-100 text-emerald-700 hover:bg-emerald-200 transition-colors disabled:opacity-50"
                                        >
                  ...
                  {QUALITY_FILTERS.map((f) => (
                    <button
                      ...
                      className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-all ${
                        qualityFilter === f.key ? "bg-brand-600 text-white shadow-sm" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                      }`}
                    >
                      {f.label}
                    </button>
                        ...
                        {qualityProducts.map((p) => {
                          ...
                          return (
                            <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="px-6 py-4 text-sm text-slate-600">{materials || "—"}</td>
                              <td className="px-6 py-4">
                                {missing > 0 ? (
                                  <span className="inline-flex rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-bold text-amber-700">{missing} missing</span>
                                ) : (
                                  <span className="inline-flex rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-bold text-emerald-700">Complete</span>
                                )}
                              </td>
                            </tr>
                          );
              ...
              <div className="space-y-6">
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                  <article className="glass-card p-6">
                    <h3 className="text-base font-bold text-slate-900 mb-4">Production Status Breakdown</h3>
                  ...
                  <article className="glass-card p-6">
                    <h3 className="text-base font-bold text-slate-900 mb-4">Product Quality Distribution</h3>
                    <div className="space-y-3">
                      {[
                        ...
                        { label: "Not Scored", count: products.filter((p) => typeof p.aiScore !== "number").length, color: "bg-slate-300" },
                      ].map((item) => {
                        ...
                        return (
                          <div key={item.label}>
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <span className="text-xs font-bold text-slate-900">{item.count} <span className="text-slate-400 font-normal">({pct}%)</span></span>
                            </div>
                            <div className="h-2 w-full rounded-full bg-slate-100">
