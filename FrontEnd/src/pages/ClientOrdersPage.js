...
const STATUS_META = {
  ...
  BLOCKED_INSUFFICIENT_STOCK:    { label: "Processing",             cls: "status-amber",   Icon: Clock,         desc: "Some items need to be produced before we can fulfill your order." },
  BLOCKED_INSUFFICIENT_MATERIALS:{ label: "Processing",             cls: "status-amber",   Icon: Clock,         desc: "We are sourcing materials to fulfill your order." },
  BLOCKED_NO_BOM:                { label: "Processing",             cls: "status-amber",   Icon: Clock,         desc: "Our team is preparing your order." },
  CONFIRMED:                     { label: "Confirmed",              cls: "status-emerald", Icon: CheckCircle2,  desc: "Your order is confirmed and scheduled for production." },
  DATE_CHANGE_REQUESTED:         { label: "Action Required",        cls: "status-sky",     Icon: AlertTriangle, desc: "We need to change your delivery date. Please review the proposal." },
  IN_PRODUCTION:                 { label: "In Production",          cls: "status-blue",    Icon: Factory,       desc: "Your order is currently being manufactured." },
  READY:                         { label: "Ready for Pickup",       cls: "status-emerald", Icon: Truck,         desc: "Your order is complete and ready for delivery!" },
  DELIVERED:                     { label: "Delivered",              cls: "status-slate",   Icon: CheckCircle2,  desc: "Your order has been delivered. Thank you!" },
                  ...
                  <div key={step.key} className="flex items-center flex-1 min-w-0">
                    <div className="flex flex-col items-center gap-1 flex-none">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-colors ${
                        done ? "bg-brand-500 text-white" : "bg-slate-100 dark:bg-slate-700 text-slate-400"
                      }`}>
                        {done ? "✓" : idx + 1}
                      </div>
            ...
            {orders.map(order => {
              ...
              return (
                <div key={order.id}
                  ...
                  className={`glass-card rounded-2xl p-5 cursor-pointer hover:shadow-md transition-all group border ${
                    needsAction ? "border-amber-300 dark:border-amber-500/40" :
